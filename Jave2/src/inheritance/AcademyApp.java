...
public class AcademyApp {
	public static void main(String[] args) {
		...
		AcademyStudent[] students =new AcademyStudent[300];//객체가 아님.. 배열임!! (AcademyStudent 객체)를 300개 만들 수 있다는거
		
		//강사정보(AcademyInstructor 객체)를 저장하기 위한 요소들이 존재하는 배열 생성
		AcademyInstruct[] instructors =new AcademyInstructor[50];
		...
		*/
		...
		//자식클래스의 생성자로 객체를 생성하면 부모 객체를 먼저 생성하고 자식 객체 생성
		//클래스의 배열 요소(참조변수)에 부모 객체 저장
		// => 배열 요소에는 부모 객체가 저장되어 있으므로 부모클래스의 메소드 호출
		// => 객체 형변환을 이용하면 배열 요소에 일시적으로 자식 객체를 저장하여 자식메소드 호출 가능
		...
		for(AcademyPerson person : persons) {
			//오버라이드 선언되지 않은 자식클래스의 메소드를 호출하기 위해서는
			//명시적 객체 형변환을 이용하여 부모클래스의 참조변수를 자식클래스로 변환하여
			//자식 객체를 일시적으로 저장해 자식클래스의 메소드 호출
			//문제점)상속관계가 아닌 클래스로 명시적 객체 형변환 할 경우 ClassCastException 발생
			//해결법)참조변수가 객체 형변환 가능한 클래스인지를 확인한 후 명시적 객체 형변환
			...
			System.out.println(((AcademyStudent)person).getCourse()+"의 학생정보 >> ");
				...
				System.out.println(((AcademyStaff)person).getDepartment()+"의 직원정보 >> ");
			}
			
			//오버라이드 선언된 자식클래스의 메소드는 묵시적 객체 형변환에 의해 부모클래스의 참조변수가
			//일시적으로 자식클래스로 변경되어 자식 객체를 참조해 자식클래스의 메소드 호출
			// => 오버라이드에 의한 다형성 : 묵시적 객체 형변환에 의해 참조변수의 자료형이
			//자식클래스로 변환되어 저장된 자식 객체에 따른 다른 메소드를 선택 호출
			person.display();
			System.out.println("==============================================================");
