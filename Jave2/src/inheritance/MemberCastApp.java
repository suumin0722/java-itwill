...
public class MemberCastApp {
	public static void main(String[] args) {
		...
		member1.display();
		System.out.println("==============================================================");
		//자식클래스 참조변수=new 자식클래스();//이런 방법도 가능하다
		// => 자식클래스의 생성자를 호출하여 객체를 생성할 경우 부모클래스의 생성자로 부모
		//객체가 먼저 생성되고 자식클래스의 생성자로 자식 객체를 생성해 자식클래스의 참조변수에는
		//자식 객체를 저장
		MemberEvent member2=new MemberEvent();
		
		//참조변수에 저장된 자식 객체를 참조하여 자식클래스의 메소드를 호출할 수 있으며
		//상속 관계에 의해 부모 객체를 참조해 부모클래스의 메소드도 호출 가능
		...
		// => 자식클래스의 생성자를 호출하여 객체를 생성할 경우 부모클래스의 생성자로 부모
		//객체가 먼저 생성되고 자식클래스의 생성자로 자식 객체를 생성해 부모클래스의 참조변수에는
		//부모 객체를 저장
		...
		//객체 형변환을 이용하면 부모클래스의 참조변수를 사용하여 자식 객체를 참조하여 자식클래스의
		//메소드 호출 가능
		...
		/* 	
		 * //자식클래스의 메소드를 많이 호출하고 싶을 때 이런 방법 사용
			MemberEvent event=(MemberEvent)member4; 
			event.setEmail("opq@itwill.xyz");
		 */

		//Cast 연산자보다 . 연산자 먼저 실행돼 이러가 발생하기 때문에 Cast 연산자가 먼저 실행되도록
		//우선 처리 연산자(괄호 연산자)를 사용 - 객체 형변환 후 자식클래스의 메소드 호출
		//자식클래스의 메소드를 많이 호출 할 필요 없을 때
