...
public class MemberEvent extends Member {
	...
	*/
	...
	public MemberEvent() {
		...
		//super(); //super 키워드가 있지만, 눈에 보이지 않는거뿐 !!! 알아서 자동으로 사용된다
	}

	//super : 자식클래스의 메소드에서 부모 객체를 참조하기 위한 키워드
	// => 자식클래스의 메소드에서 부모 객체의 필드 또는 메소드를 사용하기 위해 제공
	// => 자식클래스의 메소드에서 자식 객체의 필드 또는 메소드가 없는 경우
	//자동으로 super 키워드를 사용해 부모 객체의 필드 또는 메소드 사용 
	
	//super 키워드를 사용하는 경우
	//1. 자식클래스의 생성자에서 부모클래스의 매개변수가 작성된 생성자를 호출하여 부모 객체의
	//필드에 필요한 초기값을 저장하기 위해 사용
	//형식) super(값, 값, ...);
	...
	public MemberEvent(String id, String name, String email) {
		//super 키워드를 사용하여 부모클래스의 기본 생성자를 호출하기 위한 명령
		// => 생성자를 호출하는 명령 전에 다른 명령을 작성할 경우 에러 발생
		super(id, name);
		
		//자식클래스의 메소드에서 this 키워드로 자식 객체를 참조하여 필드 또는 메소드를
		//사용하고 자식 클래스에 필드 또는 메소드가 없는 경우 super 키워드로 부모 객체를
		//참조하여 필드 또는 메소드를 사용
	...
	*/
	...
	//[Alt]+[Shift]+[S] >> 팝업메뉴 - [O] >> 생성자 생성창 - 부모클래스의 생성자 선택 및 필드 선택 >> [Generate]클릭
	...
	//메소드 오버라이딩(Method Overriding) : 상속 관계에서 부모클래스의 메소드를 자식클래스에서
	//재선언하는 기능
	// => 부모클래스의 메소드를 자식클래스에서 사용하기 부적절한 경우 부모클래스의 메소드를
	//자식클래스에서 다시 선언
	// => 메소드를 오버라이딩 선언하면 부모클래스의 메소드(Hide Method-은닉화 처리와 유사)는 숨겨지고
	//자식클래스의 메소드만 사용
	...
	*/
	...
	// => Java 프로그램을 작성할 때 특별한 기능을 제공하기 위해 사용되는 어노테이션
