...
//형식) 클래스명 참조변수 = new 클래스명(값, 값, ...);
// => new 연산자로 클래스 내부에 선언된 생성자(Constructor)를 호출하여 객체 생성
// => 힙영역에 생성된 객체의 메모리 주소를 참조변수에 저장하여 참조변수로 객체를 참조
// => 참조변수에 저장된 메모리 주소로 객체를 참조하여 객체는 .연산자를 사용하여 객체의 필드 또는 메소드 사용



//클래스를 선언할 때 필드와 메소드에 접근 제한자를 사용하여 객체의 접근 허용 관련 설정
...
//자동차를 객체 모델링하여 작성된 클래스 - 객체 생성을 목적으로 선언된 클래스
// => 속성 : 모델명, 엔진상태, 현재속성 - 필드
...
public class Car {
	...
	// => 필드를 사용하여 메소드 내부에 작성된 명령을 사용하여 필요한 기능 구현
	void startEngine() {// 시동 온(On)
		engineStatus = true;
		System.out.println(modelName + "의 자동차의 시동을 켰습니다.");
	}
	...
	void stopEngine() {// 시동 오프(Off)
		...
		engineStatus = false;
		System.out.println(modelName + "의 자동차의 시동을 껐습니다.");
	}
	...
	//은닉화 처리된 필드를 위해 필드값을 반환하는 Getter 메소드와 필드값을 변경하는 Setter 메소드 선언
	//메소드 작성 - 캡슐화
	//캡슐화(Encapsulation) : 표현대상의 속성(필드)과 행위(메소드)를 하나의 자료형(클래스)로 
	// => 필드를 은닉화 처리하여 필드값을 보호하고 메소드를 사용하여 필드값 처리
	
	//Getter 메소드 : 클래스 외부에서 필드값을 사용할 수 있도록 필드값을 반환하는 메소드
	// => 필드의 자료형이 [boolean]인 경우 메소드의 이름 [is필드명]으로 작성
	//형식) public 반환형 get필드명(메소드의 이름)() { return 필드명;}
	// => 필드의 자료형이 [boolean]인 경우 메소드의 이름 [is필드명]으로 작성
	...
	public void setModelName(String modelName) {//매개변수를 이용하여 모델명을 변경하는 메소드
		//매개변수에 저장된 값에 대한 데이터 검증 후 필드값으로 저장
		//this : 메소드 내부에서 클래스의 객체를 표현하기 위한 키워드
		// => 필드는 this 키워드를 사용하여 표현 
