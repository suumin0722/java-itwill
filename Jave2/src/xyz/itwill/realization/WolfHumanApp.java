...
// => JDK 이상에서는 인터페이스에 정적메소드(Static Method)와 기본메소드(Default Method)도 작성
//형식) public interface 인터페이스명{
//			지료형 필드명=값; //public static final 생략 가능
...
// => 클래스는 implements 키워드를 사용하여 다수의 인터페이스를 상속받아 작성 가능 - 다중 상속
// => 자식클래스는 상속받은 모든 인터페이스의 추상메소드를 무조건 오버라이드 선언 
// => 인터페이스로 객체 생성은 불가능하지만 참조변수를 생성하여 자식클래스의 생성자로 만들어진
//객체 저장 가능 - 묵시적 객체 형변환에 의해 참조변수로 자식객체를 참조해 자식클래스의 메소드 호출 가능

//인터페이스 선언시 다른 인터페이스를 상속받아 작성 가능 - 다중 상속 가능
//형식) public interface 인터페이스명 extends 인터페이스명, 인터페이스명, ...{ }


//인터페이스를 선언하여 클래스 작성 시 상속 받는 이유
...
//2. 클래스를 선언시 반드시 만들어야 되는 메소드를 정의하기 위해 인터페이스 사용 - 작업지시서 역할을 제공
//ex) TV or Radio or SmartPhone >>  볼륨 증가, 볼륨 감소 - 인터페이스
// => 인터페이스를 상속받은 모든 자식클래스에 동일한 메소드가 선언되도록 규칙 제공
// => 클래스간의 결합도를 낮추어 시스템 변경에 따른 클래스 변경에도 기존 클레스에 대한
//영향 최소화 => 유지보수의 효율성 증가
...
public class WolfHumanApp {
	public static void main(String[] args) {
		...
		// => 객체 형변환을 이용하여 참조변수를 이용해 자식 객체를 참조해 자식클래스의 메소드 호출 가능
