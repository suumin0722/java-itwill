...
public class AjaxMemberDAO extends JdbcDAO {
	...
	//회원정보를 전달받아 AJAX_MEMBER 테이블의 행으로 삽입하고 삽입행의 개수를 반환하는 메소드
	public int insertAjaxMember(AjaxMemberDTO ajaxMember) {
		...
		try {
			...
			String sql="insert into ajax_member value(?,?,?,?)";
	...
	//아이디를 전달받아 AJAX_MEMBER 테이블에 저장된 행을 검색하여 회원정보(AjaxMemberDTO 객체)를 반환하는 메소드
	public AjaxMemberDTO selectAjaxMember(String id) {
		...
		try {
			...
			System.out.println("[에러]selecAjaxMember() 메소드의 SQL 오류 = "+e.getMessage());
		} finally {
			close(con, pstmt, rs);
		}
		return ajaxMember;
