...
/*
//DOM 트리에서 태그를 검색하여 Element 객체로 반환받아 클릭 이벤트가 발생될 경우 
//호출될 이벤트 처리 함수 등록
...
//AJAX(Asynchronous Javascript + XML) : XMLHttpRequest Javascript 객체를 사용하여 
//비동기식으로 웹프로그램을 요청해 결과를 응답받아 태그를 변경하는 기능
//XMLHttpRequest 객체(Ajax Engine) : 비동기식 통신을 사용하여 웹프로그램을 요청하여
//실행결과를 응답받아 처리하기 위한 Javascript 객체

//XMLHttpRequest 객체를 저장하기 위한 전역변수
var xhr=null;

//DOM 트리에서 태그를 검색하여 Element 객체로 반환받아 클릭 이벤트가 발생될 경우 
//호출될 이벤트 처리 함수 등록 - AJAX 기능을 사용하여 태그내용 변경
document.getElementById("btn1").onclick=function() {
	//XMLHttpRequest 객체를 생성하여 전역변수에 저장 - 준비상태에 자동으로 [0] 저장
	xhr=new XMLHttpRequest();
	
	//XMLHttpRequest 객체의 준비상태(ReadyState)가 변경되는 이벤트가 발생될 경우
	//호출될 이벤트 처리 함수 등록
	...
	// => url : 요청 웹프로그램의 URL 주소 전달 - 현재 서버에 위치한 웹프로그램 요청 가능
	// => aysnc : false( 동기식 통신) 또는 true(비동기식 통신 - 기본) 중 하나를 전달 
	xhr.open("get", "hello_two.jsp", true);//비동기식 통신 - 요청에 의한 응답 기다림 미발생
	//xhr.open("get", "hello_two.jsp", false);//비동기식 통신 - 요청에 의한 응답 기다림 발생
	
	//XMLHttpRequest 객체로 send() 메소드 호출 - 준비상태가 자동으로 [2]로 변경
	//XMLHttpRequest.send(data) : XMLHttpRequest 객체로 웹프로그램을 요청하기 위한 메소드
	// => data : 웹프로글매 요청시 전달될 값을 [이름=값&이름=값&...] 형식의 문자값으로 전달
...
//XMLHttpRequest 객체의 준비상태(ReadyState)가 변경될 때마다 호출되는 이벤트 처리 함수 - 4번 호출
