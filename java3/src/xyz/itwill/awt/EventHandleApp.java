...
//3.이벤트 소스에서 이벤트가 발생된 경우 이벤트 처리 클래스로 생성된 객체를 제공받아
//이벤트 처리 메소드가 자동 호출되어 처리되도록 설정
// => Componetn.addXXXListener(Listener 1): 컴퍼넌트에 발생된 이벤트를 처리하기 위한 이베트
//클래스의 객체를 매개변수로 전달받아 이벤트 
//처리 클래스의 객체를 매개변수로 전달받아 이벤트 처리 메소드를 자동호출되도록 추가(등록)하는 메소드
...
public class EventHandleApp extends Frame {
	...
	public EventHandleApp(String title) {
		...
		// => 이벤트 처리 클래스의 객체는 이벤트 처리 메소드를 자동으로 호출하여 이벤트 처리
