...
//키보드로 입력받은 원시데이터를  제공받아 파일로 전달하여 저장하는 프로그램 작성
...
public class FileOutputStreamApp {
	public static void main(String[] args) throws IOException {
		...
		//FileOutputStream 클래스: 파일에 원시데이터를 전달할 출력스트림을 생성하기 위한 클래스
		// => FileOutputStream(String name) 생성자를 사용하여 객체 생성
		// => name 매개변수에는 파일 출력스트림을 생성하기 위한 파일경로를 전달하여 저장
		// => 매개변수로 전달받은 위치(파일경로)에 파일이 존재하지 않는 경우 FileNotFoundException	
		// => FileNotFoundException 대신 IOException를 이용하여 예외 처리 또는 예외 전달 가능
		// => 파일 출력스트림은 매개변수로 전달받은 파일이 없는 경우 파일을 생성하여 출력스트을
		//제공하므로 FileNotFoundException이 발생된 경우 예외 처리하지 않고 예외 전달
		// => 매개변수로 전달받은 파일이 있는 경우 기존 파일에 저장된 값은 초기화 처리되고
		...
		// [true]를 전달하면 기존 내용 뒤에 새로운 내용을 추가하여 저
