...
public class PersonNumberApp {
	public static void main(String[] args) {
		...
		String idNumber;//주민번호를 저장하기 위한 변수
		//사용자로부터 입력받은 주민번호를 검증하기 위한 반복문
		//=>정상적인 주민번호를 입력받은 경우 반복문 종료
		while(true) {
			System.out.print("주민등록번호 입력 >> ");
			//키보드로 입력받은 문자열에서 앞뒤에 존재하는 불필요한 공백을 제거하여 변수에 저장
			idNumber=scanner.nextLine().trim();
			
			
			//if(idNumber.length()==14 && idNumber.charAt(6)=='-') break;
			if(idNumber.length()==14 && idNumber.indexOf("-")==6) break;
		...
		String separation=idNumber.substring(7,8);
		...
		System.out.println("생년원일 = "+birthday+", 성별 = "+gender);
	}
}
