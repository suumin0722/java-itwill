...
//Java API(Application Programming Interface) : Java 프로그램 개발에 필요한 기능을 제공하기 위해
...
//Class 클래스 : 메모리(MethodArea)에 저장된 클래스 정보(Class 객체 - Clazz)를 저장하여 필요한
//기능을 메소드로 제공하는 클래스
...
public class SystemApp {
	public static void main(String[] args) {
		//System.in : Java 프로그램에서 제공하는 표준 입력장치(키보드)에 대한 입력스트림(객체)
		Scanner scanner=new Scanner(System.in);
		
		//System.out : Java 프로그램에서 제공하는 표준 출력장치(모니터)에 대한 출력스트림(객체)
		System.out.println("정수값 입력 >>");
		int num=scanner.nextInt();
		
		if(num==0) {
			System.out.println("[메세지]프로그램을 종료합니다.");
			//System.exit(int status) : 프로그램을 실행하는 모든 스레드를 소멸시키는 메소드
			// => 프로그램을 강제로 종료하는 메소드
			System.exit(0);
		}
		...
		//System.currentTimeMillis() : 플랫폼(Platform:프로그램이 실행될 수 있는 환경을
		//제공하는 프로그램 - 운영체제)의 현재 날짜와 시간에 대한 타임스탬프를 반환하는 메소드
