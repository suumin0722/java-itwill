...
//=> 클라이언트의 접속으로 생성된 소켓은 새로운 스레드를 생성하여 독립적으로 입력/출력
//처리되도록 다중스레드 프로그램으로 작성
public class ChattingServerApp {
	
	//현재 서버에 접속중인 모든 클라이언트의 소켓정보가 저장된 List 객체를 저장하기 위한 필드
	...
	public static void main(String[] args) {
		...
		public class SocketThread extends Thread {
			...
			public void run() {
				...
				String aliasName="";
				try {
					//소켓의 입력스트림을 제공받아 대량의 문자데이터를 제공받을 수 있는 입력스트림으로
					//확장하여 필드에 저장
					in=new BufferedReader(new InputStreamReader(socket.getInputStream()));
					
					//소켓의 출력스트림을 제공받아 대량의 문자데이터를 전달할 수 있는 출력스트림으로
					//확장하여 필드에 저장
					//PrintWriter(OutputStream out, boolean autoFlush) 생성자로 객체 생성
					// => autoFlush 매개변수에 [true]를 전달하면 버퍼를 사용하지 않고 무조건
					//출력스트림으로 문자데이터 전달 - Writer.flush() 메소드 호출 생략 가능
