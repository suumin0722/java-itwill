	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {
				...
				/*
				//Socket.getOutputStream(): Socket 객체로부터 클라이언트에게 원시데이터를
				//전달할 수 있는 출력스트림(OutputStream 객체)을 반환하는 메소드
				OutputStream stream=socket.getOutputStream();
				
				//매개변수로 OutputStream 객체를 전달받아 객체를 전달할 수 있는 출력 스트림(ObjectOutputStream)으로  확장
				ObjectOutputStream out=new ObjectOutputStream(stream);
				
				//확장된 출력스트림을 이용하여 플랫폼의 현재 날짜와 시간정보가 저장된 Date  객체를
				//클라이언트에게 전달
