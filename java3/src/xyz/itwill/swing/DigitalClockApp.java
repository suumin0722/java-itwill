...
public class DigitalClockApp extends JFrame {
	...
	//플랫폼의 현재 날짜와 시간 정보를 제공받아 출력하는 스레드의 실행상태를 저장하기 위한 필드
	...
	public DigitalClockApp(String title) {
		...
		*/
		...
		new Thread(() -> {
				...
				while(true) {
					//isRun 필드값이 [true]인 경우에만 플랫폼의 현재 날짜와 시간 정보를 제공받아
					//컴퍼넌트의 문자열 변경 처리
					if(isRun) {
		...
		//JVM(Java Virtual Machine)은 이벤트를 처리하기 위한 Event-Queue 스레드 자동 생성
	...
	*/
	...
	// => 버튼을 누른 경우 플랫폼의 현재 날짜와 시간 정보를 제공받아 출력하는 스레드를 일시 중지하거나
	//재실행되도록 설정
