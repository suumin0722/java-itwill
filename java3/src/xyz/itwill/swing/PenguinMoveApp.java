...
public class PenguinMoveApp  extends JFrame {
	...
	//배경이미지를 저장하기 위한 필드
	//Image 클래스: 사진 파일(JPEG, GIF, PNG 파일 등)의 이미지 정보를 저장하기 위한 클래스
	...
	public PenguinMoveApp(String title) {
		...
		// =>ImageIcon 클래스의 ImageIcon(URL location)의 생성자 매개변수에
		//사진파일의 URL 주소를 전달받아 ImageIcon 객체 생성
		//URL: 리소스 파일(사진  파일, 소리 파일, 동영상 파일 등)의 파일 경로 또는
		//인터넷 경로를 저장하기 위한 클래스
		//Object.getClass(): 현재 실행중인 클래스의 Class 객체(메모리에 저장된 클래스)를 반환하는 메소드
		//Class.getResource(String name): 현재 실행중인 클래스의 파일 경로를 기준으로 리소스
		//파일을 읽어 리소스 파일의 경로가 저장된 URL 객체를 반환하는 메소드
		//ImageIcon.getImage(): ImageIcon 객체에 저장된 사진파일에서 이미지가 저장된 Image 객체를 반환하는 메소드
		...
		//this.addKeyListener(new PenguinMoveHandle()); => 앞에 사실 this 가 있는거임!
	...
	public void paint(Graphics g) {
		...
		// => 컴퍼넌트 or 컨테이너(ImageObserver 객체)이미지를 출력하는 메소드
