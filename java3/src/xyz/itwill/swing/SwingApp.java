...
//GUI 프로그램은 java.awt 패키지의 컴퍼넌트를 이용하여 작성하는 것보다 javax.swing 패키지의
//컴퍼넌트를 사용하여 작성하는 것을 권장 
// => javax.swing 패키지의 컴퍼넌트는 Java 프로그램에서 제공하는 컴퍼넌트 - 플랫폼에 독립적으로 동작
// => java.awt 패키지의 컴퍼넌트(컨테이너)를 구현한 클래스 이름 앞에 [J]를 붙이면 javax.swing
//패키지의 컴퍼넌트(컨테이너)를 구현한 클래스

//javax.swing 패키지의 컴퍼넌트(컨테이너)로 UI 클래스를 작성하는 방법 - AWT 프로그램과의 차이점
//1.AWT 컴퍼넌트(컨테이너)를 구현한 클래스 이름 앞에 [J]를 붙여 javax.swing 패키지의
//컴퍼넌트(컨테이너)로 구현
...
public class SwingApp extends JFrame {
	...
	public SwingApp(String title) {
		...
		//Swing 컴퍼넌트에서는 플랫폼에서 제공하는 모든 글꼴 사용 가능
		...
		// => TextArea 컴퍼넌트를 출력 전용 컴퍼넌트로 사용
		textArea.setFocusable(false);
		
		//JFrame.getContentPane(): 프레임의 Container 객체를 반환하는 메소드
		Container container=getContentPane();
		
		//JTextArea 컴퍼넌트는 폭과 높이를 벗어난 문자열 출력이 불가능 - 스크롤 미발생
		...
		// => DO_NOTHING_ON_CLOSE : 아무런 동작도 발생되지 않도록 설정하느 상수
	...
	public class TextComponentEventHandle implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(!text.equals("")) {//입력된 문자열이 있는 경우
				...
				//TextComponent.setText(String text): JTextField 컴퍼넌트 또는 JTextArea 컴퍼넌트에 입력된
				//문자열을 변경하는 메소드
				textField.setText("");
			}
		}
