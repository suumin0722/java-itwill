...
public class ListApp {
	public static void main(String[] args) {
		...
		System.out.println("==============================================================");
		//List.add(int index, E element) : List 객체에서 index 위치의 요소에 객체(값)을 삽입하여
		//저장하는 메소드
		list.add(4, "장길산");
		System.out.println("list = "+list);
		System.out.println("==============================================================");
		//List.remove(E element) : 매개변수로 전달받은 객체와 동일한 객체가 저장된 요소를
		//List 객체에서 삭제하는 메소드 - 동일한 객체가 여러개인 경우 첫번째 요소 삭제
		//list.remove("임꺽정");
		...
		System.out.println("list = "+list);
		System.out.println("==============================================================");
		//List.set(int index, E element) : List 객체에서 index 위치의 요소에 저장된 객체(값)을 변경하는 메소드
		list.set(1, "임걱정");
		System.out.println("list = "+list);
		System.out.println("==============================================================");
		//List 객체에 저장된 요소값에 대한 일괄처리 - 일반 for 구문 사용
		for(int i=0; i<list.size(); i++) {
			System.out.print(list.get(i)+" ");
		}
		System.out.println();
		System.out.println("==============================================================");
		//List.iterator() : List 객체의 모든 요소를 반복처리할 수 있는 Iterator 객체를 반환하는 메소드
		Iterator<String> iterator=list.iterator();
		
		//List 객체에 저장된 요소값에 대한 일괄처리 - Iterator 객체 사용
		while (iterator.hasNext()) {
			System.out.print(iterator.next() + " ");
		}
		System.out.println();
		System.out.println("==============================================================");
		//List 객체에 저장된 요소값에 대한 일괄처리 - 향상된 for 구문 사용
		for(String str : list) {
		...
		System.out.println();
		System.out.println("==============================================================");
		//Collections 클래스 : 컬랙션 객체의 요소를 처리하기 위한 기능을 메소드로 제공하는 클래스
		//Collections.sort(List list) : 매개변수로 전달받은 List 객체에 저장된 모든 요소값을
		//비교하여 오름차순 정렬하기 위한 메소드
		// => List 객체에 저장된 요소값(객체)을 비교하기 위해 클래스는 반드시 Comparable 인터페이스를
		//상속받아 compareTo() 메소드를 오버라이드 선언하여 필드값을 비교해 정렬될 수 있도록 처리
		Collections.sort(list);
		System.out.println("list = "+list);
		System.out.println("==============================================================");
		//List.contains(E element) : 매개변수로 전달받은 객체(값)와 같은 객체가 List 객체의
		//요소값으로 저장되어 있지 않은 경우 [false]를 반환하고 저장되어 있는 경우 [true]를 반환하는 메소드
		if(list.contains("홍길동")) {
			System.out.println("List 객체에 저장된 요소값입니다.");
		} else {
			System.out.println("List 객체에 저장되어 있지 않은 요소값입니다.");
		}
		System.out.println("==============================================================");
		...
		//list.isEmpty() : List 객체에 저장된 요소값이 있는 경우 [false]를 반호나하고 요소값이 하나도
