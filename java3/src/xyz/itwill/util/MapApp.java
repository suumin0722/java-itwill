...
public class MapApp {
	public static void main(String[] args) {
		...
		//map.put(K key, V value) : 매개변수로 전달받은 이름과 값이 저장된 엔트리를 Map 객체에
		// => Map 객체에 저장되는 엔트리의 이름(Key)은 Set 객체 형태로 저장되므로 동일한 이름(객체)의
		//엔트리는 중복 저장 불가
		map.put(1000, "홍길동");
		map.put(2000, "임꺽정");
		map.put(3000, "전우치");
		...
		System.out.println("map = "+map);
		System.out.println("==============================================================");
		//Map.get(K key) : Map 객체에서 매개변수로 전달받은 이름의 엔트리에 값(객체)를 반환하는 메소드
		// => 매개변수로 전달받은 이름의 엔트리가 Map 객체에 없느 경우 null 반환
		...
		System.out.println("==============================================================");
		//Map.values() : Map 객체에 저장된 모든 엔트리의 값(value)이 저장된 Collection 객체(List 객체)를 반환하는 메소드 
