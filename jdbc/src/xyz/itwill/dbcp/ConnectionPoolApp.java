...
// => Connection 객체의 개수 제한 가능 - 자원의 낭비를 최소화

public class ConnectionPoolApp {
	public static void main(String[] args) throws SQLException {
		//ConnectionPool 클래스는 싱글톤 클래스이므로 new 연산자로 생성자를 호출하여 객체를 
		//생성하지 못하지만, 정적 메소드를 호출하여 ConnectionPool 객체를 반환받아 사용 가능
		ConnectionPool cp=ConnectionPool.getInstance();
		
		//ConnectionPool.getConnection() : ConnectionPool 객체에 저장된 다수의 Connection 객체 중
		//하나를 반환하는 메소드 - 다른 사용자가 Connection 객체를 사용할 수 없도록 설정하는 메소드
