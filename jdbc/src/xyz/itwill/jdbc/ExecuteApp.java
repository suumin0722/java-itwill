...
//		ConnectionFactory.close(con, stmt, rs);
		...
		// => false 반환 : DML명령 또는 DDL 명령을 전달하여 실행한 경우의 반환값
		...
		if(result) {//SELECT 명령이 전달되어 실행된 경우
			//Statement.getResultSet() : Statement 객체로 전달되어 실행된 SELECT 명령의 실행결과(검색행)가 저장된 ResultSet 객체를 반환하는 메소드
			ResultSet rs=stmt.getResultSet();
			
			ConnectionFactory.close(con, stmt, rs);
			
		} else {//SELECT 명령 외의 SQL 명령이 전달되어 실행된 경우
