...
public class ResultSetMetaDataApp {
	public static void main(String[] args) throws SQLException {
		...
		//ResultSet 객체 : SELECT 명령을 전달해 실행된 검색결과(검색행)을 저장한 객체
		...
		System.out.println("=====================================================================");
		//ResultSetMetaData.getMetaData() : 검색행(ResultSet 객체)의 대한 부가적인 정보(검색대상)가
		//저장된 ResultSetMetaData 객체를 반환하는 메소드
		ResultSetMetaData rsmd=rs.getMetaData();
		
		//ResultSetMetaData.getColumnCount() : 검색행(ResultSet 객체)의 컬럼 개수를 반환하는 메소드
		...
		for(int i=1; i <=columnCount; i++) {
			...
			//ResultSetMetaData.isNullable(int columnIndex) : 첨자 위치(columnIndex)의 컬럼의 NULL 허용 유무값(0 또는 1)을 반환하는 메소드
			...
			//ResultSetMetaData.getColumnTypeName(int columnIndex) : 첨자 위치(columnIndex)의 컬럼의 오라클 자료형을 반환하는 메소드
