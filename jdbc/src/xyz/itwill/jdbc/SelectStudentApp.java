...
public class SelectStudentApp {
	public static void main(String[] args) {
		...
		try {
			...
			//커서(Cursor)를 생성하여 제공
			// => ResultSet 커서를 제공받으면 ResultSet 커서는 ResultSet 객체의 BOF(Befor Of File) 영역에 위치되도록 설정
			//ResultSet.next() : ResultSet 커서를 다음행으로 이동하는 메소드
			// => false 반환 : ResultSet 커서 위치에 처리행이 없는 경우 반환값 - ResultSet 커서가 EOF(End Of Field) 영역에 위치한 경우
			// => true 반환 : ResultSet 커서 위치에 처리행이 있는 경우 반환
			if(rs.next()) {//ResultSet 커서를 다음행으로 이동하여 처리될 행이 있는 경우
				System.out.println("[메세지]검색된 학생정보가 있습니다.");
				
				//ResultSet 객체에 저장된 학생정보를 차례대로 제공받아 출력 처리하기 위한 반복문
				// => ResultSet 객체에 저장된 검색행의 개수가 불확실하므로 while 구문 사용
				// => if 구문에서 ResultSet 커서를 다음행으로 이미 이동하였으므로 do~while 구문을 사용
				do {
					...
					//검색행의 컬럼값은 컬럼 자료형에 상관없이 getString() 메소드를 호출하여 문자열로 반환받아 처리 가능
					...
					//날짜형인 컬럼값ㅇ르 문자열로 반환받으면 [yyyy-MM-dd HH:mm:ss] 패턴의 문자열로 반환
					// => 날짜만 출력되도록 문자열을 분리하여 출력
					System.out.println("생년월일 = "+birthday.substring(0,10));
					System.out.println("==============================");
				} while (rs.next());//ResultSet 커서를 다음행으로 이동하여 처리될 행이 있는 경우 반복문 실행하며
									//처리될 행이 없는 경우 반복문 종료
				
			} else {//ResultSet 커서를 다음행으로 이동하여 처리될 행이 없는 경우 - 검색된 행이 없는 경우
