...
public class SqlMinusApp {
	public static void main(String[] args) throws SQLException, IOException {
		...
		//키보드로  SQL 명령을 입력받아 DBMS 서버에 전달하여 실행하고 실행결과를 제공받아 출력처리하기 위한 반복문
		while (true) {
			...
			//키보드 입력값이 (대소문자 구분 없이)[exit]인 경우 반복문 종료
			if(sql.equalsIgnoreCase("exit")) break;
			
			try {
				//if 구문의 조건식 대신 Statement 객체의 execute() 메소드를 호출하여 DBMS 서버에
				//SQL 명령을 전달하여 실행하고 실행된 명령을 구분하여 원하는 기능을 제공하기 위한 선택문
				if(stmt.execute(sql)) {//SELECT 명령을 전달하여 실행한 경우
					...
					if(rs.next()) {//검색행이 있는 경우
						...
						do {
							//ResultSet 커서 위치의 처리행을 모든 컬럼값을 문자열로 반환받아 저장
							for(int i =1 ; i <=columnCount; i++) {
				...
				} else {//SELECT 명령을 제외한 SQL 명령(INSERT,DELETE,UPDATE)을 전달하여 실행한 경우
					int rows=stmt.getUpdateCount();
					System.out.println(rows+"개의 행을 "+sql.substring(0, 6).toUpperCase()+"하였습니다.");
				}
				
			} catch (SQLException e) {
				//키보드로 입력받아 DBMS 서버에 전달되어 실행된 SQL 명령이 잘못된 경우에 발생된
				//SQLException에 대한 예외 처리 명령 작성\]\
