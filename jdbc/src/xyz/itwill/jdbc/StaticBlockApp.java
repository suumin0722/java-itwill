...
public class StaticBlockApp {
	public static void main(String[] args) throws ClassNotFoundException {
		/*
		//1.JVM은 ClassLoader 프로그램을 이용하여 클래스(Class 파일)을 읽어 메모리의 Method 영역에 저장
		...
		*/
		...
		//Class.forName(String className) : 문자열로 표현된 패키지 경로가 포함된 클래스를
		//매개변수로 전달받아 ClassLoader 프로그램을 사용하여 클래스를 메모리에 저장하는 메소드 
		// => 메모리에 저장된 클래스의 정보가 저장된 Class 객체를 생성하여 반환
		// => 문자열로 전달받은 클래스가 없는 경우 ClassNotFoundException 발생되므로 반드시 예외 처리
