...
//JDBC 프로그램은 기본적으로 자동 커밋 기능(AutoCommit)이 활성화되어 있어 DML 명령을 전달하여 실행하면 무조건 자동으로 커밋 처리 
// => 프로그램 실행시 예외가 발생될 경우 예외 발생 전에 전달되어 실행된 DML 명령은 롤백 처리 불가능
//JDBC 프로그램에서는 자동 커밋 기능(AutoCommit)을 비활성화 처리하고 프로그램 실행시 예외가 발생되지 않은 경우 커밋 처리하고
...
public class TransactionControlApp {
	...
	try {
		...
		//con.setAutoCommit(boolean autoCommit) : 자동 커밋 기능(AutoCommit)이 활성화 상태(true) 또는
		//비활성화 상태(false)로 변경하는 메소드
		...
		if(rows>0) {//삭제행이 있는 경우
			System.out.println("[메세지]"+rows+"명의 정보가 변경되었습니다.");
			} else {//삭제행이 없는 경우
				System.out.println("[메세지]해당 학번의 학생정보를 찾을 수 없습니다.");
			}
		
			//Connection.commit() : 커밋 처리하는 메소드
