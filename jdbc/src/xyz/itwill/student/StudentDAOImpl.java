...
public class StudentDAOImpl extends JdbcDAO implements StudentDAO {
	...
	public static StudentDAOImpl get_dao() {
		return _dao;
	}
	...
	public List<StudentDTO> selectStudentByName(String name) {
		...
		try {
			...
			while(rs.next()) {
				...
				student.setPhone(rs.getString("phone"));
				student.setAddress(rs.getString("address"));
				student.setBirthDay(rs.getString("brithday").substring(0, 10));
	...
	public List<StudentDTO> selectStudentList() {
		...
		try {
			...
			// => 검색행이 있는 경우 반복문의 명령으로 검색결과를 Java 객체로 변환하여 List 객체의
			//요소로 추가되도록 처리
	...
	/*
	 * //이름을 전달받아 STUDENT 테이블에 저장된 해당 이름의 학생의 인원수를 검색하여 반환하는 메소드 public int
	 * selectStudentCountByName(String name) { Connection con=null;
	 * PreparedStatement pstmt=null; ResultSet rs=null; int count=0; try {
	 * con=getConnection();
	 * 
	 ...
	 * } catch (SQLException e) {
	 * System.out.println("[에러]insertStudent() 메소드의 SQL 오류 = " + e.getMessage()); }
	 * finally { close(con, pstmt, rs); } return count;
	 * 
	 * }
	 */
}
