...
<%
	...
	if(pageName==null) {//전달값이 없는 경우
		...
		//요청해 실행 결과(HTML 문서)를 응답받아 출력 - 리다이렉트 이동 
...
<%-- forward 태그 : 요청JSP 문서에서 다른 JSP 문서로 스레드를 이동하여 
스레드가 이동된 JSP 문서로 클라이언트에게 응답하기 위한 기능을 제공하는 태그 - 포워드 이동 --%>
<%-- => 요청JSP 문서가 아닌 스레드가 이동된 JSP 문서가 클라이언트에게 응답 처리 --%>
<%-- => 요청JSP 문서에서 다른 JSP 문서로 스레드가 이동될 때 요청 JSP 문서의 request 객체와
