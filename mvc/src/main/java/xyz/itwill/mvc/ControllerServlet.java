...
public class ControllerServlet extends HttpServlet {
	...
	//서블릿 클래스가 객체로 생성된 후 한번만 호출하는 메소드 - 초기화 작업
	// => 클라이언트가 서블릿을 최초로 요청한 경우 서블릿 클래스를 객체로 한번만 생성
	// => [web.xml] 파일의 servlet 엘리먼트의 하위 엘리먼트(load-on-startup)를 설정하면
	...
	public void init(ServletConfig config) throws ServletException {
		...
		*/
		...
		// ServletConfig.getInitParameter(String parameterName) : [web.xml] 파일의
		// init-param 엘리먼트로 제공된 값을 반환하는 메소드
		...
		for (Object key : properties.keySet()) {
			...
			//Properties 객체에 저장된 엔트리의 값(Value)을 반환받아 저장 - 모델 클래스(ModelClass)
			String actionClass = (String)properties.get(key);
			
			try {
				//모델 클래스로 모델 객체를 생성하여 엔트리로 만들어 Map 객체에 추가
				//리플렉션(Reflection) : 프로그램의 명령 실행 시 Class 객체(Clazz)로 객체를
				...
				//Class.getDeclaredConstructors() : Class 객체의 생성자가 저장된 
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => 요청 처리 메소드가 선언된 모델 역할의 클래스 작성 - 인터페이스를 상속받아 작성
		...
		/*
		 * 이 방법보다는 Map객체 사용이 나음
		//객체 저장 가능
		Action action=null;
		
		//클라이언트의 요청정보를 비교하여 요청을 처리하기 위한 모델 역할의 클래스로 객체를 생성하여
		//인터페이스 참조변수에 저장
