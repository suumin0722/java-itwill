...
// => 단위 프로그램(모듈 - Module) : 컴포넌트(Component)
// => 다수의 DAO 클래스의 메소드를 호출하여 데이터 처리에 필요한 기능을 제공 - DAO 모듈화
...
public class UserinfoService {
	...
	//아이디를 전달받아 USERINFO 테이블에 저장된 행을 검색하여 회원정보를 반환하는 메소드
	// => 매개변수로 전달받은 아이디의 회원정보가 USERINFO 테이블에 없는 경우 인위적 예외 발생
	...
	// => 인증 실패 시 인위적 예외 발생 - 예외가 발생하지 않는 경우 인증 성공
