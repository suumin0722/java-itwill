...
<body>
	...
	<%-- EL을 사용하여 Date 객체를 제공받아 출력하면 자동으로 Date.toString() 메소드 호출 --%>
	<p>now = ${now }</p>
	 
	 <%-- formatDate 태그 : Date 객체에 저장된 날짜와 시간을 원하는 패턴의 문자열로 변환하여 제공 --%>
	...
	<p>now(날짜 full) = <fmt:formatDate value="${now }" type="date" dateStyle="full"/></p>
	<p>now(날짜 short) = <fmt:formatDate value="${now }" type="date" dateStyle="short"/></p>
