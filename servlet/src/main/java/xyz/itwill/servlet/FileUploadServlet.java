...
//입력페이지(fileupolad.html)에서 전달된 문자값과 파일의 이름을 HTML 문서로 만들어 클라이언트에게
//전달하여 응답하는 서블릿
// => 사용자로부터 입력받아 전달된 파일은 서버 디렉토리 저장 - 파일 업로드(File Upload)
...
public class FileUploadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 
		 * //[multipart/form-data] 형태로 전달되는 문자값은 HttpServletRequest 객체의 //getParameter()
		 * 메소드를 호출하여 반환 불가능 String uploader=request.getParameter("uploader");
		 */
