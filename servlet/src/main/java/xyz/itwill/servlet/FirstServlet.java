...
//@WebServlet("/first.itwill"): 서블릿 클래스를 서블릿으로 등록하고 URL 패턴을 매핑하기 위한 어노테이션
// => [web.xml] 파일의 servlet 엘리먼트와 servlet-mapping 엘리먼트로 제공되는 기능과 동일
//@WebServlet 어노테이션 속성을 사용하여 서블릿의 이름과 URL 패턴을 매핑 설정
// => name 속성 : 서블릿의 이름을 속성값으로 설정 - 속성을 생략하면 클래스의 이름을 서블릿의 이름으로 자동 설정
// => value 속성 : 서블릿을 요청하기 위한 URL 패턴을 속성값으로 설정 - 속성 생략 불가능
// => value 속성 외에 다른 속성이 없는 경우 속성값만 설정 가능
...
public class FirstServlet extends HttpServlet {
	...
	// => 클라이언트 요청에 대한 처리 명령과 처리 결과를 저장한 응답파일을 생성하는 명령 작성 
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//클라이언트 요청에 대한 처리 결과를 저장하기 위한 파일 형태 변경 및 응답 파일을 생성하기 위한
		//출력스트림을 반환받아 저장
		...
		//서버 플랫폼의 현재 날짜와 시간이 저장된 Date 객체를 생성하여 저장
		Date now=new Date();
		
		//날짜와 시간의 패턴 정보를 저장한 SimpleDateFormat 객체를 생성하여 저장
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy년 MM월 dd일 HH시 mm분 ss초");
		
		//SimpleDateFormat 객체로 format() 메소드를 호출하여 매개변수로 전달받은 Date 객체를
		//SimpleDateFormat 객체의 저장된 패턴의 문자열로 반환받아 저장
