...
// => 클라이언트 요청에 의한 서블릿 객체가 이미 생성되어 있는 경우 새로운 서블릿 객체를 생성하지
//않고 기존 서블릿 객체를 사용하여 요청 처리 메소드 호출
...
// => 서블릿은 컨테이너에 의해 관리되는 객체(Bean) - 컴퍼넌트(Component) : JavaBean
...
public class LifeCycleServlet extends HttpServlet {
	...
	//서블릿 클래스를 객체로 생성하기 위한 생성자(Constructor)
	// => 객체를 생성하면서 실행될 초기화 명령 작성 - 필드 초기화
	// => 기본 생성자 안만들어도 됨. 알아서 처리함.
	...
	public void init(ServletConfig config) throws ServletException {
		System.out.println("### LifeCycleServlet 클래스의 init() 호출 - 객체 생성 ###");
		//name="홍길동";
	...
	// => 일반적으로 이 메소드는 만들지 않음! C언어의 경우 메모리 정리를 위해 사용하지만, 
	//자바는 가비지 컬렉터가 자동으로 정리해주기 때문에 호출 하지 않아도 됨
	@Override
	public void destroy() {
	...
	// => 요청에 대한 처리 명령 작성 - 데이터 처리 및 응답 파일 생성 - 요청 처리 메소드
