...
// => [WEB-INF]와 [META-INF] 폴더는 WAS 프로그램만 접근 가능한 숨겨져 있는 폴더 - 클라이언트 요청 불가 
...
public class SendImageServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => 이진파일을 생성하기 위해 원시데이터를 전달하는 ServletOutputStream 객체 반환
		ServletOutputStream out=response.getOutputStream();
		
		//서버에 저장된 이미지 파일의 시스템 경로를 반환받아 저장
		//HttpServletRequest.getServletContext() : ServletContext 객체를 반환하는 메소드
		// => ServletContext 객체 : 웹자원(WebContext)을 관리하기 위한 객체 - WAS 프로그램을 객체로 표현한 객체
		//ServletContext.getRealPath(String contextPath) : 매개변수로 전달받은 웹자원의 파일 시스템 경로를
		//반환하는 메소드
		String imageFilePath=request.getServletContext().getRealPath("/WEB-INF/Koala.jpg");
		System.out.println("imageFilePath = "+imageFilePath);
		// => 콘솔에 이렇게 출력됨 => imageFilePath = C:\Users\itwill\apache-tomcat-9.0.84\webapps\servlet\WEB-INF\Koala.jpg

		
		//이미지 파일을 읽기 위한 파일 입력스트림 생성(FileInputStream 객체) 생성
		// => FileInputStream 클래스의 생성자 매개변수에 이미지 파일의 경로 전달
		FileInputStream in=new FileInputStream(imageFilePath);
		
		//입력스트림으로 원시데이터를 읽어와 출력스트림에게 전달 - 반복 처리
