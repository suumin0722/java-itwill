...
//클라이언트로부터 [JSESSIONID]라는 이름의 쿠키값을 제공받지 못한 경우 서버에 새로운 새션을
...
public class SessionServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//HttpServletRequest.getSession() : 바인딩 처리되어 제공된 세션을 HttpSession 객체로 반환하는 메소드 
		// => 새로운 세션을 생성하여 바인딩하거나 기존 세션을 트랙킹하여 바인딩 처리
		//HttpSession 객체 : 연결 지속성을 제공하는 객체(값)를 저장하기 위한 객체 - 세션
		//브라우저마다 세션은 다 다름
		...
		out.println("<hr>");
		//HttpSession.isNew() : HttpSession 객체를 트랙킹하여 바인딩한 경우 [false]를 반환하고
		//HttpSession 객체를 생성하여 바인딩한 경우 [true] 반환
		if(session.isNew()) {
		...
		// => 세션의 유지시간을 변경하지 않으면 기본적으로 30분 동안 유지되도록 설정
		...
		// => HttpSession 객체(세션)에 연결 지속성을 제공하기 위한 값(객체)을 저장하기 위한 메소드
		// => 매개변수에는 값(객체)를 구분하기 위한 속성명과 연결 지속성을 제공하기 위한 속성값 전달
		// => 매개변수로 전달받은 속성명과 같은 이름의 속성값이 세션에 이미 저장되어 있는 경우 덮어 씌우기 - 속성값 변경
		// => 동일 클라이언트는 세션을 사용하여 모든 웹프로그램에게 객체를 저장하여 제공 - 객체 공유
		session.setAttribute("now", new Date());
		
		//HttpSession.getAttribute(String attributeName) : 매개변수로 전달받은 이름으로
		//HttpSession 객체(세션)에 저장된 값(객체)을 저장하기 위한 메소드
		// => HttpSession 객체에 저장된 모든 속성값은 Object 객체로 반환되므로 반드시 명시적 객체 형변환 사용
		...
		//HttpSession 객체(세션)에 저장된 값(객체)을 삭제하는 메소드
