...
- 페이지번호, 검색대상, 검색단어 전달 --%>
<%-- => [게시글개수] 태그의 입력값을 변경한 경우 [/review/review_list.jsp] 문서를 요청하여 페이지 이동 
- 페이지 번호, 게시글개수, 검색대상, 검색단어 전달 --%>
...
- 글번호, 페이지번호, 게시글개수, 검색대상, 검색단어 전달 --%>
<%
	//게시글 검색 기능에 필요한 전달값(검색대상과 검색단어)을 반환받아 저장
	String search=request.getParameter("search");//검색대상(컬럼명)
	...
	int pageSize=10;//게시글개수- 전달값이 없는 경우 저장된 초기값 설정
	if(request.getParameter("pageSize")!=null) {//전달값이 있는 경우
		pageSize=Integer.parseInt(request.getParameter("pageSize"));
	}

	//검색정보(검색대상과 검색단어)를 전달받아 REVIEW 테이블에 저장된 게시글 중 검색대상의 컬럼에
	//검색단어가 포함된 게시글의 개수를 검색하여 반환하는 ReviewDAO 클래스의 메서드 호출
	int totalReview=ReviewDAO.getDAO().selectTotalReview(search, keyword);//검색된 게시글의 총개수
	
	//전체 페이지의 개수를 계산하여 저장
	//int totalPage=totalReview/pageSize+totalReview%pageSize==0?0:1;
	int totalPage=(int)Math.ceil((double)totalReview/pageSize);//페이지의 총개수
	...
	//마지막 페이지의 게시글의 종료 행번호가 검색 게시글의 총개수보다 많은 경우 종료 행번호 변경
	...
	// => 게시글이 비밀글인 경우 로그인 상태의 사용자가 게시글 작성자이거나 관리자인 경우에만 권한을 제공
	...
	// => 검색된 게시글의 총개수가 91개인 경우 >> 1Page : 91 ~ 82, 2Page : 81, 3Page : 71
