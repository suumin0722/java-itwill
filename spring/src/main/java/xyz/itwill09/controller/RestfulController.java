...
// => 페이지 요청에 대한 실행 결과를 클라이언트에게 XML 또는 JSON 형식의 텍스트 데이터로 응답 처리
...
public class RestfulController {
	...
	*/
	...
	//문자값(텍스트 데이터)로 클라이언트에게 전달하여 응답하는 어노테이션
	// => 반환값을 ViewResolver 객체를 사용하여 뷰로 변환하여 응답 처리하지 않고 요청 처리 메소드의
	//반환값으로 응답 처리
	// => @ResponseBody 대신 ResponseEntity 클래스를 요청 처리 메소드의 반환형으로 작성하여 응답 처리
	...
	// => GET 방식으로 페이지를 요청한 경우 리퀘스트 메세지 몸체부가 비어 있으므로 @RequestBody 사용 불가능
	// => 비동기식 방식으로 페이지 요청시 JSON 형식의 테스트 데이터로 전달된 값을 매개변수로 저장하기 위해 사용
	...
	*/
	...
	public ResponseEntity<RestMember> restMember() {
		try {
			...
			//클라이언트에게 응답코드 200과 실행결과를 텍스트 데이터로 응답 처리
